-- $Id: lups.lua 3347 2008-12-02 02:01:51Z jk $
---------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------
--
...
local glRenderer = gl.GetString(GL_RENDERER)
...
local function DetectCard(vendor,renderer)
  ...
  NVseries  = ((isNvidia)and(
                 ...
                 (renderer:find(" 3") and 3) or math.huge
               )) or 0
...
local function addVisibleFx(fx)
  fx.visible = true
      ...
      for unitID,UnitEffects in pairs(Units) do
        if (not UnitEffects[1]) then
          ...
          if (unitID>-1) then
            ...
            local dx,dy,dz = spGetUnitDirection(unitID)
            if (dx==nil) then --// UnitID isn't valid anymore, remove all correspondend effects
              for _,fx in pairs(UnitEffects) do
              ...
              local unitActive  = spGetUnitIsActive(unitID)
              local x,y,z       = spGetUnitViewPosition(unitID)

              local unitRadius     = spGetUnitRadius(unitID) + 40
              --local unitVisible  = spIsUnitVisible(unitID)
              local unitVisible100 = spIsSphereInView(x,y,z,unitRadius+100)
              local unitVisible300 = spIsSphereInView(x,y,z,unitRadius+300)

              --// check effects
              for _,fx in pairs(UnitEffects) do
                fx.visible=false
                if (unitActive or (not fx.onActive)) then

                  if (fx.Visible) then
                    fx.visible = fx:Visible()
                  elseif (fx.radius) then
                    fx.visible = ( unitVisible100 and fx.radius<=100  )or
                                 ( unitVisible300 and fx.radius<=300 )
                  else
                    fx.visible = (unitVisible100)
                  end
