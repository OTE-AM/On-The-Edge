-- $Id: RadarParticles.lua 3171 2008-11-06 09:06:29Z det $
-----------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------

local RadarParticles = {}
RadarParticles.__index = RadarParticles
...
function RadarParticles.GetInfo()
  return {
    name      = "RadarParticles",
...
function RadarParticles:CreateParticleAttributes(up, right, forward, partpos,n)
...
function RadarParticles.BeginDraw()
  glUseShader(billShader)
  glBlending(GL_ONE, GL_ONE_MINUS_SRC_ALPHA)
end

function RadarParticles.EndDraw()
  glTexture(0,false)
...
function RadarParticles:Initialize()
  ...
  if (billShader==nil) then
    print(PRIO_MAJOR,"LUPS->RadarParticles: Critical Shader Error: " ..gl.GetShaderLog())
    return false
  end
...
function RadarParticles:Finalize()
...
function RadarParticles:Update(n)
  self.frame  = self.frame + n
end

-- used if repeatEffect=true;
function RadarParticles:ReInitialize()
  self.frame = 0
...
function RadarParticles:CreateParticle()
...
function RadarParticles:Destroy()
  gl.DeleteList(self.dlist)
  --gl.DeleteTexture(self.texture)
end

function RadarParticles:Visible()
...
function RadarParticles.Create(Options)
  local newObject = MergeTable(Options, RadarParticles.Default)
  setmetatable(newObject,RadarParticles)  --// make handle lookup
